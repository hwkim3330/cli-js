...
function cborToJsonDelta(cborData, sidInfoMap, typeTable, sidInfo, parentSid = null) {
  ...
  // Buffer - convert to base64 string for YAML
  if (Buffer.isBuffer(cborData)) {
    return cborData.toString('base64');
  }

  // Uint8Array - convert to base64 string
  if (cborData instanceof Uint8Array) {
    return Buffer.from(cborData).toString('base64');
  }

  // Other objects - return as-is
  return cborData;
}
